...
### Интерактивный запуск через отдельное окно

В корне репозитория добавлен сценарий `run_datamatrix_prompt.bat`, который запускает отдельное окно командной строки и пошагово
запрашивает данные, необходимые для распознавания:

```powershell
run_datamatrix_prompt.bat
```

Скрипт предложит указать `client_id`, `client_secret`, путь к файлу изображения и желаемую предустановку качества.
Если приложение уже опубликовано (`build.bat`), будет запущен скомпилированный exe, в противном случае — `dotnet run` из исходников.
